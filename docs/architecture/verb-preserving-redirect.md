...
- **Required of the implementer:** build the library project itself, not just the test project, before returning the work. That compiles both targets and is the guard that exists today.
- **Recommended, not folded in:** a PR-time workflow step building `Pooshit.Http/Pooshit.Http.csproj`. It is the durable fix and it is a CI change in a design about redirects — §9 raises it rather than smuggling it in.
...
| "the mechanism cannot break `netstandard2.0`" | D6.1 | a later edit swapping the cast for `HttpStatusCode.PermanentRedirect` | **Yes**, and it is the likely one. This is why D6.1 does not stop at the mechanism and names the build as the actual guard. |
...
6. **Build the library project against both target frameworks** — not only the test project (D6.1). This is the only guard the repo has for `netstandard2.0` before a tag push.
