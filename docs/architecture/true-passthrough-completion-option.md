...
| T-R5 | Follow-redirects **off**, a 302 | No second request — pinned by `Get_Redirect302_DefaultOptions_Throws`; ~~the 3xx flows to validation~~ — **struck, superseded by #8316:** an unfollowed 3xx now raises rather than passing validation, and `HttpServiceStatusBandTests` covers that path. **Resolved 2026-09-22 (#14588).** |
| T-R6 | Follow-redirects on, a status outside 3xx | No second request — pinned by `Get200_FollowRedirectsTrue_IssuesOneRequest`. ~~a non-redirect status~~ — **condition narrowed 2026-09-22 (#14588)**, which is what made the row checkable: it now means *outside 3xx*, the reading T-R4 left uncovered. The other reading, *a 3xx no redirect arm acts on*, is its own cell and is pinned separately by `Get_NonFamily3xx_FollowRedirectsTrue_Throws`. |

> **AUDIT 2026-09-22 — T-R4 was not the only row in this table that cannot be checked (DiVoid #14528).**
>
> After T-R4 was retired, this table was swept row by row against the test project. **Of six rows: one was never guarded, one cannot be resolved as written, and one went unguarded for its entire life before an unrelated change half-superseded and half-pinned it.**
>
> - **T-R4** — confirmed never written. Nothing ever asserted `NotSupportedException` for a 307, which is why removing the throw broke nothing.
> - **T-R5** — **two claims, and they have diverged.** *No second request* is now pinned exactly, by **`Get_Redirect302_DefaultOptions_Throws`** (`HttpServiceStatusBandTests.cs`), which uses a **302**, passes no options, and asserts `handler.Requests, Has.Count.EqualTo(1)`. That is an exact match on all three axes of this row's condition — status, option state, and the typed path this section is about. Two near misses, named because picking between them is the whole point of the convention: `Get308_FollowRedirectsFalse_Throws` pins the same behaviour under the same option state but on a **308**, and `GetResultLess_Redirect302_Throws` uses the right status but the **result-less** path, which never enters the redirect hop §11.7 describes. *The 3xx flows to validation* is **superseded**: since #8316 an unfollowed 3xx raises rather than passing validation. Chronology, established with `git log -S`: that test, `HttpServiceStatusBandTests.cs` and the rest of the follow-redirects-off coverage **all first appear in `68f7b1d`** (PR #21, 2026-09-22), so the row was unguarded for its entire life and is guarded now — by a change made for an unrelated reason.
> - **T-R6** — **unresolvable as written, and that is the finding.** The nearest candidate is `Get_NonFamily3xx_FollowRedirectsTrue_Throws` (`HttpServiceStatusBandTests.cs`), which pairs `FollowRedirects = true` with 300/304/305/399 and asserts `Requests.Count == 1` directly. Whether that *is* T-R6's guard depends on whether *"a non-redirect status"* means **not 3xx at all** (a 200 — nothing asserts that; it is pinned only indirectly, because `Get_Redirect302_FollowRedirectsTrue_SingleHop_StillSucceeds` would exhaust its response sequence if the library hopped on a 200) or **a status no redirect arm acts on** (300/304/305 — pinned exactly). **The row's branch condition is too loose to decide whether its own guard exists**, which is the same defect class the naming convention was written to kill, one level up: an unfalsifiable *condition* rather than an unfalsifiable *identifier*.
>
> **T-R1, T-R2 and T-R3 are confirmed real** — `AbsoluteLocationResolvesToAbsoluteUrl` / `RelativeLocationResolvesAgainstRequestUri`, `StreamingOptionCarriesThroughRedirectHop`, and `SupersededRedirectResponseIsDisposed` — verified by reading the bodies, not by matching the names.
>
> **This note's first version was itself the defect it documents, with the sign flipped.** It struck T-R5 as *"no guard was ever written"* when one had existed for four commits, on the strength of `grep "FollowRedirects = false"` returning nothing across `Http.Tests`. It still returns nothing — **`false` is the flag's default, so every test exercising that path reaches it by passing no options at all.** The predicate measured a spelling nobody writes and was mistaken for a measurement of behaviour. **A table that denies coverage which exists is no safer than one that claims coverage which does not:** delete that `Has.Count` assertion tomorrow and this table would say there was never anything there to lose. **Greppability is necessary and not sufficient — the predicate has to name the behaviour, not a token that happens to co-occur with it.**
>
> ~~**This note deliberately adds no tests.** Whether T-R6 deserves a guard for the 200 reading, and whether T-R5's superseded clause should simply be retired, are questions about the current design rather than docs fixes; they are filed (#14588) rather than answered here.~~
>
> **RESOLVED 2026-09-22 (#14588).** Struck text retained per #11228 Lesson 3. **T-R6 kept**, its condition narrowed to *outside 3xx*, and `Get200_FollowRedirectsTrue_IssuesOneRequest` written for it. **T-R5 kept**, clause one left pointing at its live guard and clause two struck. Neither row remains in the annotated-but-undecided state, which is the state that produced this note.
>
> **And the "pinned indirectly" claim in this note was itself wrong, measured rather than reasoned.** It held that `Get_Redirect302_FollowRedirectsTrue_SingleHop_StillSucceeds` would exhaust its response sequence if the library hopped on a 200. It does not: with the redirect branch made to match `OK`, that test **passes** and only the new guard fails. The sequence is never exhausted because a single-hop implementation stops after the 302 and never tests the 200 against the condition at all — the exhaustion argument assumes a loop this code does not have. So T-R6's 200 reading was not weakly guarded; it was **unguarded**. That is the third claim in this one table established by reasoning about a fixture rather than running it, after the T-R4 row and the `FollowRedirects = false` grep above.
