...
| T-R5 | Follow-redirects **off**, a 302 | No second request — **now pinned**, by `Get_Redirect302_DefaultOptions_Throws`; ~~the 3xx flows to validation~~ — **superseded by #8316.** See the note below. |
| T-R6 | Follow-redirects on, a non-redirect status | No second request — **cannot be resolved as written**; the branch condition is ambiguous. See the note below. |
