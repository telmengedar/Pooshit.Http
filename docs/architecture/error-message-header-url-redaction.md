...
**One field outside the core specification qualifies on both clauses and is still excluded: WebDAV's `Destination` (RFC 4918 §10.3).** Its whole value is a URI-reference, and as a request header it is reachable — a caller can set it and the dump would walk it. It is out on YAGNI: the library offers no WebDAV affordance, a `Destination` names a target resource on the server already being addressed rather than carrying a credential, and no consumer is asking. Naming it here rather than omitting it is the point — the rule admits it, so the exclusion is a judgement and not an oversight, and a consumer who needs it covered has the `SensitiveHeaders` route of the next paragraph today and a one-line set entry if that is ever wrong.
