...
**The knob a caller has for that cost is coarse, and that is a limit rather than a feature.** The only remedy for an unwanted redaction is `SensitiveQueryParameters.Remove(entry)`, and an entry is broad: removing `key` to make `?keyword=` legible also stops `secretkey`, `accesskey` and `AWSAccessKeyId` being redacted, and removing `sig` to make `?design=` legible costs the coverage set out immediately below. No per-name exclusion list is introduced for this: it would be a second enumeration surface with no named consumer (#1136 §6, YAGNI), and unlike the matching set it would fail in the under-redaction direction when it drifted.

**What `Remove("sig")` costs exactly.** `sig` is the entry most likely to be removed, and `signature` ships redundantly (§3.3, D6a) to soften that. It softens it **partially**, and the split is stated here once, precisely, because a reassurance that is stronger on paper than in fact is worse than no reassurance:

| After `SensitiveQueryParameters.Remove("sig")` | |
|---|---|
| `X-Amz-Signature`, `X-Goog-Signature`, `signature_v4`, any `*signature*` spelling | **still redacted**, via `signature` |
| a bare `?sig=` — Azure Storage SAS | **no longer redacted** |

**The residual is structural, not a shortfall in the remedy.** Substring matching means an entry only ever covers names **at least as long as itself** — the base fact the whole rule rests on. So the only entries that would reach a bare `?sig=` are substrings of `sig` itself: `si`, `ig`, `g` and the like. Each of those genuinely works — adding `si` does redact `?sig=` again, and §6's mutation row proves it rather than assuming it — but each also matches an enormous benign population (`si` alone hits `design`, `assignee`, `version` and `session`), which is strictly worse than the over-redaction the caller removed `sig` to escape. **The shortest entry in a set is therefore the one it has no *useful* rescuer for**, and `sig` is that entry as well as being, for Azure, the *entire* parameter name. A caller who removes `sig` and needs Azure SAS covered puts `sig` back and accepts `?design=` being redacted, or adds the specific names their own upstream uses. §6 case 7a pins both rows.
...
**What is left of it is narrow, and stating it required measuring both shipped runtimes rather than reasoning about one.** The package targets `netstandard2.0` and `net8.0`, and `Uri.ToString()` does not decode the same escapes on both. Measured directly, not inferred:

| Escape in a parameter name | .NET Framework 4.8 (`netstandard2.0` consumer) | .NET 8 |
|---|---|---|
| `%5F` `_` · `%2D` `-` · `%6B` `k` — unreserved | decoded | decoded |
| `%20` space | decoded | decoded |
| `%2F` `/` · `%3F` `?` · `%26` `&` · `%3D` `=` · `%3A` `:` · `%40` `@` · `%2B` `+` | **decoded** | **preserved** |
| `%23` `#` · `%25` `%` | preserved | preserved |
...
**The default set — eight entries, all credential words rather than parameter names:**

`token` · `key` · `secret` · `password` · `signature` · `sig` · `auth` · `credential`
...
**Substring matching makes an entry that contains another entry redundant — and one of them ships anyway, deliberately.** `apikey` and `accesstoken` were removed when segmentation was withdrawn: they existed only to reach compounds with no boundary, and `key` and `token` now reach those directly. Re-adding either leaves the suite green, which is the evidence that the membership is dead rather than an argument that it is.

**`signature` is redundant by the same arithmetic and is shipped regardless, because redundancy is not the only thing worth buying.** No name containing `signature` fails to contain `sig`, so while `sig` is present the entry cannot change a verdict — its presence is provably free. What it buys is the case where `sig` is *not* present. `sig` is the widest-colliding entry in the set (`design`, `assignee`, `resign`, `consignment`), which makes it the entry a consumer is most likely to `Remove` — and that removal would otherwise take `X-Amz-Signature`, `X-Goog-Signature` and `signature_v4` with it, **silently**. It takes Azure's bare `?sig=` either way — no entry can *usefully* rescue the shortest entry in the set, for the reason §3.2 gives — so what `signature` buys is the long-form family, which is most pre-signed-URL usage, and not the whole of it. The first revision of this section discharged that with prose telling the caller to add `signature` back. A prose remedy fails in exactly the way this whole feature exists to stop: the caller who does not read it gets no warning, no log line, and a leaked credential. One redundant string converts that into nothing to remember, and §6 pins the scenario rather than describing it.
...
The exact consequence of removing `sig`, both what survives and what does not, is stated once in §3.2 and pinned by §6 case 7a.
...
**D6a — `signature` ships redundantly, so that reducing the set cannot silently drop the long-form signature family.** §3.3. Under substring matching `signature` is subsumed by `sig` and cannot change a verdict while `sig` is present; it is in the set for the configuration where `sig` has been removed. Rejected alternative: omit it and tell the caller in prose to add it back — rejected because a prose remedy fails silently, which is the failure mode the whole feature exists to remove, and because the entry costs nothing measurable (one string, verdict-neutral by construction).

**Scope of the claim, narrowed after review.** An earlier wording said reducing the set *"cannot silently reduce coverage"*, which overclaimed: `Remove("sig")` still drops Azure Storage SAS's bare `?sig=`, and no entry can usefully rescue it (§3.2 — an entry reaches only names at least as long as itself, so the only candidates are substrings of `sig`, every one of which over-matches far worse than `sig` did). The decision survives the narrowing on the arithmetic rather than on the rhetoric: `signature` costs one verdict-neutral string and strictly reduces the blast radius of the most likely destructive edit, from *the entire signature family* to *Azure alone*. Strictly better at zero cost is still worth taking; it is simply not the whole rescue the first wording implied. Reversal cost: one string and one assertion; removing it re-widens that blast radius, so the reversal is not free even though the addition was.
...
| 7a | A caller removes `sig`: `?design=` becomes legible, `X-Amz-Signature` **stays redacted**, and a bare `?sig=` **does not** | D6a and §3.2's table, both rows. This is the scenario the redundant `signature` entry exists for and the only case in which it is load-bearing — dropping it from the set fails here and nowhere else — and the third assertion keeps the doc from claiming a rescue that does not happen. Two mutations pin the two halves: dropping `signature` reds this test alone, and adding `si` — a substring of `sig` — also reds it, which is what makes §3.2's *"the only candidates are substrings of `sig`"* a measured statement rather than an argument |
...
| 6 | A **URL inside a dumped header value** — `Location`, `Content-Location`, `Referer` | **Closed since** by `error-message-header-url-redaction.md` (#9940) for `Location` and `Referer`; `Content-Location` proved unreachable and `Link` stays open there. Open and out of scope when this design was written: Under `HeaderDumpMode.Redacted` these names are not in `SensitiveHeaders`, so their values print verbatim, and Meta's `paging.next` arrives as exactly such a value. Closing it means changing header rendering and deciding which header names are URL-valued — a separate decision on a surface this task excludes. |
...
| Trade-offs named explicitly | **Pass.** §3.2 (`key` and `sig` over-redact `sortkey`, `keyword`, `author`, `assignee`, `design`), §3.2 (the coarse removal knob; the narrowed percent-encoding limit), §3.3 (`session` and `code` excluded), §3.4 (the `;` separator leak), §4 (path segments unaddressable), §7 route 6 (the `Location` route left open, closed since by #9940), §5 D3 (no value-shape safety net). |
