...
**The residual is structural, not a shortfall in the remedy.** A redundant entry can only rescue names **longer than itself**, so the shortest entry in the set has no possible rescuer — and `sig` is both the shortest entry and, for Azure, the *entire* parameter name. Nothing addable to the set covers a bare `?sig=` once `sig` is gone. A caller who removes `sig` and needs Azure SAS covered puts `sig` back and accepts `?design=` being redacted, or adds the specific names their own upstream uses. §6 case 7a pins both rows.
...
**Scope of the claim, narrowed after review.** An earlier wording said reducing the set *"cannot silently reduce coverage"*, which overclaimed: `Remove("sig")` still drops Azure Storage SAS's bare `?sig=`, and no entry can rescue it (§3.2 — a redundant entry reaches only names longer than itself, and `sig` is the shortest). The decision survives the narrowing on the arithmetic rather than on the rhetoric: `signature` costs one verdict-neutral string and strictly reduces the blast radius of the most likely destructive edit, from *the entire signature family* to *Azure alone*. Strictly better at zero cost is still worth taking; it is simply not the whole rescue the first wording implied. Reversal cost: one string and one assertion; removing it re-widens that blast radius, so the reversal is not free even though the addition was.
