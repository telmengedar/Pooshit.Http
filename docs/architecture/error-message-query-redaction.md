...
**`signature` is redundant by the same arithmetic and is shipped regardless, because redundancy is not the only thing worth buying.** No name containing `signature` fails to contain `sig`, so while `sig` is present the entry cannot change a verdict — its presence is provably free. What it buys is the case where `sig` is *not* present. `sig` is the widest-colliding entry in the set (`design`, `assignee`, `resign`, `consignment`), which makes it the entry a consumer is most likely to `Remove` — and that removal would otherwise take `X-Amz-Signature`, `X-Goog-Signature`, `signature_v4` and the Azure SAS signature with it, **silently**. The first revision of this section discharged that with prose telling the caller to add `signature` back. A prose remedy fails in exactly the way this whole feature exists to stop: the caller who does not read it gets no warning, no log line, and a leaked credential. One redundant string converts that into nothing to remember, and §6 pins the scenario rather than describing it.
...
**D6a — `signature` ships redundantly, so that reducing the set cannot silently reduce coverage.** §3.3. Under substring matching `signature` is subsumed by `sig` and cannot change a verdict while `sig` is present; it is in the set for the configuration where `sig` has been removed. Rejected alternative: omit it and tell the caller in prose to add it back — rejected because a prose remedy fails silently, which is the failure mode the whole feature exists to remove, and because the entry costs nothing measurable (one string, verdict-neutral by construction). Reversal cost: one string and one test; removing it re-opens the silent path, so the reversal is not free even though the addition was.
...
| 7a | A caller removes `sig`: `?design=` becomes legible **and** `X-Amz-Signature` stays redacted | D6a. This is the scenario the redundant `signature` entry exists for, and the only case in which that entry is load-bearing — dropping it from the set fails here and nowhere else |
