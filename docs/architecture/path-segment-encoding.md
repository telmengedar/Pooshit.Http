...
|---|---|---|---|
| B1 | Two independent causes. **The added parameter:** the argument in the base position is not a `string` (`Rest.Path(id, "x")`), the call passes an `object[]` variable (`Rest.Path(parts)`), or it passes nothing — and the base is the *second* argument of both `PathQuery` overloads, so `PathQuery(q, parts)` fails there rather than on the first. **The rename** of `elements` into `baseUrl` / `segments`: a named argument (`Rest.Path(elements: parts)`, `Rest.PathQuery(querystring: q, elements: parts)`) no longer binds, and a method-group conversion (`Func<object[], string> f = Rest.Path`) no longer matches. | **Compile error** — CS1503 / CS7036 / CS1739 / CS0123 | Name the base explicitly, or update the argument name. |
