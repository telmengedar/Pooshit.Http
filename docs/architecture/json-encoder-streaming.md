...
**The assertion is on the difference between the two figures rather than on their ratio, and the threshold comes from the difference's own spread.** A constructor allocating a constant measures a difference of zero however large that constant is; a constructor allocating a share of what it serialised measures a difference proportional to the 288,000 bytes by which the document grew. The bound is **24,000 bytes** — four bytes for each of the 6,000 elements separating the two documents, against a document costing **48.0 bytes an element** — so what passes is an implementation whose constructor allocation grows by under a twelfth of what it reads. Measured on the shipped encoder over **40 cold observations**, one per suite run and each the single measurement the test itself makes, the difference ranges **−744 to +640 bytes**. That is a sample rather than an excursion bound, and it does not hold: an independent 285-observation campaign across four runtime configurations reached **+5,360** in the *quiet* stream, so the bound clears the largest observed excursion by **4.5×**. The tail is not load-driven - three-way contention topped out at +1,608 - and Server GC is dramatically quieter, 41 of 45 observations at exactly zero, so the exposure is the Workstation GC that `dotnet test` defaults to. Under deliberate stress — twenty back-to-back 8,000-element encodes inside one test while the rest of the suite runs — the tail reached **+7,416** once in 300 observations, which the bound still clears by 3.2×. Every axis below was run three times against the suite, which collects 427 with a measurement probe added and 426 as shipped:
