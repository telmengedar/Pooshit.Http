...
Row 9 stays for the reason the subsumption paragraph below gives, and because it says the simpler thing directly: an implementation that hands the transport a block at or above the cap is defective on the wire, whatever it read to build it.

**The hole that is left, named rather than closed.** Rows 9 and 10 bound how much of the document may be **held before it is written**. They say nothing about what is **kept after it is written**. They cannot see an implementation which streams within the read-ahead bound *and also retains every byte it has already emitted* — `Json.WriteAsync` through a tee that forwards to the transport and appends to a `MemoryStream`, for instance. QA #14667 §1 built exactly that, axis C24: **425 of 425 green**. Such an implementation satisfies both rows and still allocates O(n), and the only thing that separates it is the allocation measurement this section could not make stable. **A reliable one needs a dedicated process, which makes it a benchmark rather than a suite guard.** It is filed as **#14661**, and is not pretended closed here.

**What is bounded and what is not, without rounding up.** Row 10 bounds the largest run an implementation may materialise before writing it to **under an eighth of the document**. It does not bound it to a constant, and it is measured at one document size — 96,001 bytes — against one package version. An implementation that materialises in blocks below that bound and releases each one passes, and that is the intended outcome rather than a residual escape: **bounded** retention is the property worth wanting, and what #10071 reports is **unbounded** retention. An implementation that retains what it has already emitted also passes, and that one is #14661.

**Subsumption is not a reason to delete rows from this table.** On the 33-axis mutation population of #14657 §4, five of these guards have red-sets that are strict subsets of another's: rows **1**, **2**, **3** and **8**, and the `(1)` case of row **11**. Row 9 joins them this round, against the axes in the table above. They are all kept. Subsumption is a property of the axis population rather than of the guards — row 9 was subsumed at 30 axes and not at 33, and is subsumed again now that row 10 asserts more — so a guard deleted against today's axis set is a guard unavailable when tomorrow's mutation arrives. Rows 5 and 6 — the two that pin literals at representative sizes, 85,000 and 85,001 bytes — are subsumed by nothing, and row 5 is itself a *subsumer*, of row 1. What the subsumed five are worth is not a literal but a name: each states one property directly, so when one reds, the message says which property broke instead of leaving that to be inferred from a subsumer's failure.
