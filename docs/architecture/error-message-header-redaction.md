...
`Http.Tests/HttpServiceHeaderRedactionTests.cs`. Axes varied independently: mode (each of the three, plus unset), request-side vs response-side headers, the service-level name set (added / removed), case-insensitivity of the match, per-call-vs-service resolution in both directions, null options, and which overload the options bag reaches.
...
`CallMode_ReachesEveryOverloadThatValidatesStatus` fans a per-call `Full` across every public overload that can reach `CheckHttpResponse`, because the call sites are the axis most likely to be plumbed incompletely — #8299's "nothing is picked up automatically" hazard scales with the number of sites. When this document was written `Post(url, options)` was deliberately absent from the fan, being the one member that did not validate status at all; #8317 closed that, so it validates like its seven siblings and is in the fan, and the count is nine rather than eight. The two `Send` cases carry the marker on the `HttpRequestMessage` rather than in `HttpOptions.Headers`, because `Send` takes a pre-built request and never runs `CreateRequest`, so nothing the options bag contributes at request-build time reaches it — it honours the per-call mode but applies neither option headers nor the token provider (#9609).
...
The mutation evidence — every mutation applied, and the tests each one kills — lives in DiVoid: session log #9599 for the implementation-side runs, review #9602 and re-review #9608 for the independently re-measured tables.
